...
| Observation | `observeState()`, `observeIdentityState()`, `observeConnectionState()`, `observeUnreadCount()` | The event channel exposes native-derived lifecycle state and includes `unreadCount` only after an account-authorised conversation-list response; no inbox or credential state is retained in Dart. |
| Types | Session, identity, connection, unread, push-result, error-code, and retry-directive types | Typed boundary values and native-safe error mapping. |

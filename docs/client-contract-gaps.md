...
| Attachment conversation binding | Attachment intent requires `conversationId`, upload completion must precede chat send, and `ChatRequest` carries attachments but no `conversationId`; the contract does not state that an attachment receipt controls chat routing. | A client cannot request an intent for an image on the first message of a new conversation. For an existing conversation, it cannot prove that the completed attachment and chat send target the same conversation. | Keep attachment sending disabled and do not declare its capabilities. Do not create a placeholder text message, guess a conversation ID, or assume receipt-based routing. Local picker and upload-state work may continue with synthetic fixtures only. | The server/contract owner must define new-conversation attachment bootstrap and existing-conversation receipt/chat binding before either send flow is enabled. |
...
| Notification coverage | `message_available` delivery exists for AI replies, Operator replies, and ticket-message replies. Direct playbook/proactive inserts and ticket-resolution lifecycle messages are not enqueued; anonymous delivery currently requires an unavailable contact association. | Those customer-visible messages may appear after foreground refetch but do not reliably generate mobile push. | Keep push payload handling generic and refetch-authorised. Do not invent notification types or manufacture local notifications. | Server must enqueue the existing payload for every customer-visible message and define anonymous installation authorization; status-only ticket events still require an approved contract decision. |
...
| --- | --- | --- |
| Attachments | Blocked by the attachment conversation-binding wire gap above; native send must not be implemented yet. | Keep picker/upload capabilities undeclared until the contract is reconciled and implementation, fixture, conformance, and device evidence pass. |
